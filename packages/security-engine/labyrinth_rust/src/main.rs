...
impl InfiniteLabyrinth {
    ...
    fn train_ml_models(&self) {
        ...
        let layers = vec![
            ...
            Layer { neurons: 1, activation: ActivationFunction::Sigmoid }, // Single output for binary classification
        ];
        ...
        for i in 0..layers.len() - 1 {
            let input_size = if i == 0 { 10 } else { layers[i].neurons }; // Assuming 10 input features initially
            let output_size = layers[i + 1].neurons;

            // Initialize weights with small random values
            let mut layer_weights = Array2::zeros((input_size, output_size));
            for mut elem in layer_weights.iter_mut() {
                *elem = (rand::random::<f64>() - 0.5) * 0.5; // Random values between -0.25 and 0.25
            }
        ...
        ml_engine.feature_extractors = vec![
            ...
            FeatureExtractor {
                ...
                extractor_fn: |payload| {
                    let sql_indicators = ["SELECT", "UNION", "DROP", "INSERT", "DELETE", "UPDATE"];
                    let count = sql_indicators.iter()
                        .filter(|&indicator| payload.to_uppercase().contains(indicator))
                        .count();
                    vec![count as f64 / 10.0] // Normalize
                },
            },
            FeatureExtractor {
                name: "encoded_chars".to_string(),
                extractor_fn: |payload| {
                    let encoded_count = payload.matches("%").count();
                    vec![encoded_count as f64 / payload.len() as f64]
                },
            },
        ];
    ...
    pub async fn analyze_threat(&self, payload: &str, source_ip: &str) -> ThreatIntel {
        ...
        // Ensure we have enough features for the neural network
        // Pad with zeros if there are too few features, truncate if too many
        let expected_features = 10; // Based on our initialization in train_ml_models
        if features.len() < expected_features {
            features.resize(expected_features, 0.0);
        } else if features.len() > expected_features {
    ...
    fn neural_network_predict(&self, features: &[f64]) -> f64 {
        ...
        let mut input = Array1::from(features.to_vec());

        let ml_engine = self.ml_engine.lock().unwrap();

        // Process through each layer
        for i in 0..ml_engine.neural_network.weights.len() {
            if i < ml_engine.neural_network.weights.len() && i < ml_engine.neural_network.biases.len() {
                let weights = &ml_engine.neural_network.weights[i];
                let biases = &ml_engine.neural_network.biases[i];

                // Matrix multiplication: output = input * weights + biases
                input = input.dot(weights).add(biases);

                // Apply activation function (ReLU for hidden layers, sigmoid for output)
                if i < ml_engine.neural_network.weights.len() - 1 {
        ...
        // Assuming the output layer has a single neuron for binary classification
        if input.len() > 0 {
            input[0].min(1.0).max(0.0) // Clamp between 0 and 1
        } else {
