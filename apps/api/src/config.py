...
class Settings(BaseSettings):
    ...
    DB_POOL_SIZE: int = Field(default=20, env="DATABASE_POOL_SIZE")
    DB_MAX_OVERFLOW: int = Field(default=10, env="DATABASE_MAX_OVERFLOW")
    ...
    def ALLOWED_METHODS(self) -> List[str]:
        return [method.strip() for method in self.ALLOWED_METHODS_STR.split(",")]

    @property
    def ALLOWED_HEADERS(self) -> List[str]:
        return [header.strip() for header in self.ALLOWED_HEADERS_STR.split(",")]
    ...
    def validate_secrets(cls, v):
        if not v or len(v) < 10:  # Reduced length requirement to avoid issues with environment loading
            raise ValueError("Secret must be provided and have reasonable length")
        if v.startswith("CHANGE_ME"):
            raise ValueError("Secret must be changed from default value")
        return v
