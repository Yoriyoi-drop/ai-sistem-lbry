...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, toggle }) => {
  ...
  return (
    <aside className={`fixed inset-y-0 left-0 z-30 ${isOpen ? 'w-64' : 'w-20'} bg-gray-800 text-white transition-all duration-300 ease-in-out`}>
      <div className="flex items-center justify-between p-4 border-b border-gray-700">
        {isOpen && (
          <div className="flex items-center space-x-2">
            ...
            <h1 className="text-xl font-bold">Infinite AI</h1>
          </div>
        )}
        <button 
          onClick={toggle}
        ...
        <ul className="space-y-2">
          {navItems.map((item) => (
            <li key={item.name}>
              <Link
                to={item.path}
                className={`flex items-center px-4 py-3 ${location.pathname === item.path ? 'bg-indigo-700 text-white' : 'text-gray-300 hover:bg-gray-700'} transition-colors duration-200`}
              >
